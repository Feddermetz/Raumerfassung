...
def CalcDerivative(scanLine, minDist):
    ...
    for i in range(1, len(scanLine) - 1):
        if (y_val[i + 1] >= rd.minValidSensorData and y_val[
            i + 1] >= rd.minValidSensorData):  # TODO: was wird gemacht wenn eine ungültige Distanz gemessen wird?
            derivative = (y_val[i + 1] - y_val[i - 1]) / (
                        2 + 5 * rd.angleCorrection)  # Berechnung über Differenzenquotien
...
class DataManager(object):
    ...
    def Split_Data(self, newData):
        ...
        ussData = (
        (newData[11]), (newData[0]), (newData[1]), (newData[2]), (newData[3]), (newData[4]), (newData[5]), (newData[6]),
        newData[7], newData[8])
    ...
    def CreateTestData(self):  # Testdaten erzeugen(einfachster Fall, kurzer Flur rechteckig mit Datenschwankungen)
        ...
        self.test_data.append(
            [start0, start45, start90, start135, start180, start225, start270, start315, start360, startmposleft,
             startmposright, counter])
