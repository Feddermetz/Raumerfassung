...
void loop() {
  ...
  if (bt.available()) {
    ...
    switch (inDatC) {
      case 'f':
        FahreDistanzCm(10, 60, 'f');
        break;
      case 'b':
        FahreDistanzCm(10, 60,  'b');
        break;
      case 'l':
        DreheRoboterRunTurns(messwinkel, 'l');
        break;
      case 'r':
        DreheRoboterRunTurns(messwinkel, 'r');
        break;
      case 'a':
        drivingMode = 'a';
        break;
...
void automode_steps() {
  ...
  while (abstandsDaten[0] > minAbstandVHLR) {
    startzeit = millis();
    startDistanzHintenUss = uss_h.distanceCm();
    FahreDistanzCm(20, 60 ,'f');
