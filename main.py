...
#newVfrom kivy.garden.matplotlib.backend_kivyagg import FigureCanvasKivyAgg
...
class Mapping(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        map = self.ids.map
        #newV map.add_widget(FigureCanvasKivyAgg(plt.gcf()))
    ...
    def draw_data(self, dt):
        ...
        if self.slam_mode == 'ekf':
            plot = run_ekf_slam()
            #newVself.ids.map.add_widget(FigureCanvasKivyAgg(plot.gcf()))
        elif self.slam_mode == 'graph':
            print("Graph Slam started...")
